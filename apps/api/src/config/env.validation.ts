...
function requireValue(env: Record<string, unknown>, key: string): string {
  const value = env[key];
...
function optionalValue(env: Record<string, unknown>, key: string): string | undefined {
  const value = env[key];
...
function parsePort(env: Record<string, unknown>): number {
  const port = Number(optionalValue(env, 'PORT') ?? '3000');
...
function validateProduction(env: Record<string, unknown>): void {
  ...
  requireValue(env, 'PAYMENT_WEBHOOK_SECRET');
}

export function validateEnv(env: Record<string, unknown>): Record<string, unknown> {
  const nodeEnv = optionalValue(env, 'NODE_ENV') ?? 'development';
  ...
  const otpHashSecret = requireValue(env, 'OTP_HASH_SECRET');
  ...
  parseBoolean(env, 'SWAGGER_ENABLED', nodeEnv !== 'production');

  if (nodeEnv === 'production') {
    validateProduction(env);
  }

  const storageProvider = optionalValue(env, 'STORAGE_PROVIDER') ?? 'cloudinary';
